use cpal::{available_hosts, traits::DeviceTrait, SampleFormat, SupportedStreamConfigRange};
    ...
    fn show(&mut self, ui: &mut Ui, on_save: impl FnOnce(), on_cancel: impl FnOnce()) {
        Modal::new(Id::new("Audio Input Chooser"))
            .show(ui.ctx(), |ui| {
                ...
                let current_selected_device_name = device.name()
                ...
                let supported_configs_range: Vec<SupportedStreamConfigRange> = device.supported_input_configs()
                    .expect("Selected device has no supported input configs")
                    .filter(|config| config.sample_format() == SampleFormat::F32)
                    .collect();
                let selected_config_name = format!("Channels: {}, Rate: {}", config.channels, config.sample_rate.0);
                ...
                ComboBox::new("audioinput_config", "Configuration")
                    ...
                    .show_ui(ui, |ui| {
                        for config in supported_configs_range {
                            let config_display = format!("Channels: {}, Rate: {}", config.channels(), config.max_sample_rate().0);
                            ui.selectable_value(&mut selected_config, config.with_max_sample_rate().config(), config_display);
                        }
                    });
